...
SESSION_FILE = '/tmp/session.json' # File untuk menyimpan sesi agar tidak hilang di Vercel
...
def fb_process():
    ...
    # Simpan ke file agar tidak hilang di Vercel
...
def handle_tcp_lobby(client_socket, addr):
    try:
        ...
        if not data: return
        session = load_session()
        account_bytes = build_account_info(session["account_id"], "")
